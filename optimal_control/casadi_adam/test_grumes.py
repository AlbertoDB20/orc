...
def plot_infinity(t_init, t_final, DO_PLOT):
    r = 0.1
...
def define_running_cost_and_dynamics(opti, X, U, S, W, N, dt, x_init,
    ...
    for k in range(N):
        ...
        # :nq because I need positions of the joints
        ee_pos = fk(X[k][:nq])
        ee_des = cs.vertcat(
        ...
        # Cost for the tracking of the trajectory
        cost += 10**2 * cs.sumsqr(ee_pos - ee_des)

        # TODO: Add velocity tracking cost term
        # nq: because I need only velocities
        cost += w_v * X[k][nq:].T @ X[k][nq:]

        # TODO: Add actuation effort cost term
        cost += w_a * U[k].T @ U[k]

        # Csnstraint to w to be constant
...
def define_terminal_cost_and_constraints(opti, X, S, c_path, r_path, w_final):
    ...
    # Final cost for the tracking of the trajectory
    w_p = 10**2
    cost += w_p * cs.sumsqr(ee_pos - ee_des)

    
    # Terminal cost - euclidean distance
    cost += w_final  * cs.sumsqr(X[0] - X[-1])
...
if __name__ == "__main__":
    ...
    if ( PLOT ):
        ...
        ee_vel = np.gradient(ee, dt, axis=1)

        plt.figure(figsize=(10, 4))
        for i in range(3):
            plt.plot(tt, ee_vel[i, :], label=f'EE vel {i}', alpha=0.7)
