...
def savefig():
    global fig_id
    filename = os.path.join(save_folder, f"fig_{fig_id:02d}.png")
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    fig_id += 1

# ====================== Problem Parameters ======================

PLOT = True     # Set to True to enable plotting of results
N_REP = 3      # Number of repetitions of the simulation display
...
def define_running_cost_and_dynamics(opti, X, U, S, W, N, dt, x_init,
    ...
    for k in range(N):
        ...
        # Enforce the coincidence between s and the end-effector position
        ee_des = cs.vertcat(
          c_path[0] + r_path * np.cos(2 * np.pi * S[k]),
          c_path[1] + 0.5 * r_path * np.sin(4 * np.pi * S[k]),
          c_path[2]
        )

        # Cost term to track the desired trajectory
        w_p = 10**2
        cost += w_p * cs.sumsqr(ee_pos - ee_des)

        # Constrain W to be constant and equal to 1/N
        opti.subject_to( W[k] == 1.0 / (N*dt) )
...
def define_terminal_cost_and_constraints(opti, X, S, c_path, r_path, w_final):
    ...
    ee_pos_final = fk(X[-1][:nq])

    # TODO: Constrain ee_pos to lie on the desired path in x, y, z at the end
    ee_des_pos_final = cs.vertcat(
      c_path[0] + r_path * np.cos(2 * np.pi * S[-1]),
      c_path[1] + 0.5 * r_path * np.sin(4 * np.pi * S[-1]),
      c_path[2]
    )
    ...
    # Cost term to track the desired trajectory
    w_p = 10**2
    cost += w_p * cs.sumsqr(ee_pos_final - ee_des_pos_final)

    # Implement a terminal cost that penalizes the distance between the initial and final state with a weight w_final = 1 --> log_w_final = 0.
    cost += w_final * cs.sumsqr(X[-1] - X[0])

    return cost 


# NB: dt is now a decision variable, removed from function arguments
def create_and_solve_ocp(N, nx, nq, lbx, ubx, x_init,
    ...
    # X are the states, U the controls, S the path variables, W the auxiliary variables for path progression speed, dt the time step
    running_cost = define_running_cost_and_dynamics(opti, X, U, S, W, N, dt, x_init, c_path, r_path, w_v, w_a, w_w, tau_min, tau_max)
    ...
    return sol, X, U, S, W, dt    # Return also dt
...
if __name__ == "__main__":
    ...
    # w_v, w_a, w_w, w_final = 10^−6, 10^−6, 10^−6, 10^−6
